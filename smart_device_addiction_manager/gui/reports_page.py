...
class ReportsPage(ctk.CTkFrame):
    def __init__(self, master, database, productivity_service, **kwargs):
        ...
        ctk.CTkLabel(self, text="Reports & Visualization", font=HEADER_FONT).pack(anchor="w", padx=PADDING, pady=PADDING)

        filters = ctk.CTkFrame(self)
        filters.pack(fill="x", padx=PADDING)

        self.date_entry = ctk.CTkEntry(filters, placeholder_text="Date (YYYY-MM-DD)")
        self.date_entry.pack(side="left", padx=5, pady=8)
        self.month_entry = ctk.CTkEntry(filters, placeholder_text="Month (YYYY-MM)")
        self.month_entry.pack(side="left", padx=5, pady=8)

        ctk.CTkButton(filters, text="Load Date", command=self.load_by_date).pack(side="left", padx=5)
        ctk.CTkButton(filters, text="Load Month", command=self.load_by_month).pack(side="left", padx=5)
    ...
    def load_by_date(self):
        ...
        if frame.empty:
            ...
            axes[0].bar(per_app.index, per_app.values)
    ...
    def load_by_month(self):
        ...
        if frame.empty:
            ...
            axes[1].plot(list(productivity.keys()), list(productivity.values()), color="green", marker="x")
